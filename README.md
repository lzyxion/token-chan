...
**벤더 선택 규칙**은 `지금 작업 중 → 마지막으로 움직인 세션 → 오늘 사용량 1위`입니다. 두 CLI 를 번갈아 쓰면 근거가 초 단위로 뒤집혀 게이지가 깜빡이므로, 한 번 바꾸면 **5초는 유지**합니다. 특정 벤더로 **고정**하려면 설정 → 게이지에서 고르거나, **게이지 로고를 클릭**해 자동 → Claude → Codex → AGY 순으로 전환합니다 (고정 중에는 라벨 끝에 핀 아이콘).
