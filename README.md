...
```
평상시                        캐릭터에 마우스를 올리면
  [logo]                        [logo] AGY · gem-3.6-flash · 작업 중
   ◕                             ◕ 컨텍스트 16%
   ○                             ○ 한도 정보 없음
   ○                             ○ 한도 정보 없음
  ▁▃▅▁▁                         ▁▃▅▁▁
...
| 링 1 | 컨텍스트 소진율 |
| 링 2·3 | **그 벤더의** 공식 한도 (짧은 창부터). Claude 는 세션 5h + 주간, Codex free 는 월간 하나, AGY 는 없음 |
| 가로 바 | 리셋까지 남은 시간 — 다 차면 리셋(=좋음)이라 단계 색 대신 파랑 고정 |

라벨은 **호버할 때만** 뜹니다. 그래서 평상시 벤더를 알리는 건 로고 하나뿐이고, 링 색은 이미 소진율 단계(60%/85%에서 주황·빨강)에 쓰고 있어 벤더를 실을 수 없습니다.
...
**점선 링은 0%가 아니라 "그 벤더가 안 알려주는 값"입니다.** 채운 링과 같은 모양이면 소진율 0%로 읽히므로 모양 자체를 다르게 했습니다. 링 개수는 벤더가 바뀌어도 3개로 고정인데, 줄 수가 변하면 열 높이가 널뛰어 캐릭터 세로 중심이 흔들리기 때문입니다.
