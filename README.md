...
10:15:27  mtime=09:55:03  size=325952   ← +71KB, mtime 그대로
```

크기는 즉시 갱신되므로 **직전 관측과 비교해 크기나 mtime 이 달라졌는지**를 봅니다(`live.rs` 의 `WatchTracker`). stat 한 번으로는 알 수 없어 라이브 스레드가 회차 사이에 값을 들고 있습니다. mtime 신선도 판정도 그대로 남겨 뒀습니다 — 파일이 막 닫힌 직후처럼 그쪽만 잡히는 경우가 있어 둘의 합집합이 어느 하나보다 넓습니다. 확인은 `cargo run -p usage-core --example watch`. AGY 에는 `presence/<uuid>.lock` 이 있어 그쪽이 나아 보이지만 **쓸 수 없습니다** — 실측에서 agy 프로세스가 하나도 없는데 lock 파일 4개가 그대로 남아 있었습니다(종료 시 정리 안 함).
