...
```
오늘  1.6M  $4.28        91일  42.3M  $128
오늘 · 입력 91K · 출력 5.6K · 캐시 325K
```

큰 숫자를 하나만 두면 바로 아래 91일 잔디와 기간이 섞여 **전체 합계로 오해**하기 쉽습니다. 둘을 나란히 놓아 기간을 못 박았고, 기간 합계는 통계 페이지에 원래 있어야 할 값이기도 합니다(전에는 잔디를 눈으로 훑어야 총량을 짐작할 수 있었습니다). 기간 합계는 `summary.daily` 를 프론트에서 더합니다 — 일별 값이 이미 다 와 있어 서버 왕복을 늘릴 이유가 없습니다.

잔디는 GitHub과 같은 배치(열=주, 행=요일)로 **13주(91일)**를 보여줍니다. 색이 진할수록 많이 썼다는 뜻이고 **경고가 아니라서** 패널의 단계 색(주황·빨강)이 아닌 초록 4단계를 씁니다. 칸에 마우스를 올리면 날짜·토큰·비용이 뜹니다.
