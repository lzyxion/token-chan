...
/// 발견된 설치본 하나 (계정 소속·집계 포함 여부 포함)
#[derive(serde::Serialize)]
pub struct ScanRoot {
    pub source: String,
    pub path: String,
    /// 그 아래에서 발견된 트랜스크립트 수 — 경로만 맞고 데이터가 없는 경우를 구분한다
    pub files: usize,
    /// 자동 탐지가 아니라 설정의 "추가 스캔 경로"에서 온 것인지
    pub extra: bool,
    /// 이 설치본이 속한 계정 (이메일 또는 계정 id 앞자리)
    pub account: String,
    /// 계정이 집계에 포함돼 있는지 — 끄고 켜는 건 우클릭/트레이의 "연결된 계정"에서
    pub enabled: bool,
    /// 표준 위치가 아니라 마커 스캔으로 찾아낸 설치본인지
    pub discovered: bool,
}

/// 소스마다 트랜스크립트 확장자가 다르다 — Claude·Codex 는 `.jsonl`,
/// Antigravity 는 대화 하나가 SQLite 파일 하나(`.db`)다.
fn count_files(root: &std::path::Path, ext: &str) -> usize {
...
/// 발견된 설치본 목록 — 어떤 환경에서 떴든 뭘 보고 있는지 확인할 수 있게 한다.
/// (자동 탐지는 프로세스 환경에 좌우되므로 눈으로 확인할 수단이 필요하다)
#[tauri::command]
pub fn get_scan_roots(state: State<'_, AppState>) -> Vec<ScanRoot> {
    let (overrides, extras) = {
        let s = state.settings.lock().unwrap();
        let mut e: Vec<std::path::PathBuf> = vec![];
        e.extend(s.extra_claude_homes.iter().map(std::path::PathBuf::from));
        e.extend(s.extra_codex_homes.iter().map(std::path::PathBuf::from));
        e.extend(s.extra_antigravity_homes.iter().map(std::path::PathBuf::from));
        (s.accounts_enabled.clone(), e)
    };
    let accounts = { state.accounts.lock().unwrap().clone() };

    let mut out = vec![];
    for a in &accounts {
        let enabled = crate::monitor::account_enabled(a, &overrides);
        for i in &a.installs {
            let root = i.transcript_root();
            let files = match i.source {
                usage_core::model::Source::Codex => ["sessions", "archived_sessions"]
                    .iter()
                    .map(|s| count_files(&root.join(s), "jsonl"))
                    .sum(),
                usage_core::model::Source::Antigravity => {
                    count_files(&root.join("conversations"), "db")
                }
                _ => count_files(&root, "jsonl"),
            };
            out.push(ScanRoot {
                source: format!("{:?}", i.source).to_lowercase(),
                files,
                extra: extras.iter().any(|e| i.home.starts_with(e)),
